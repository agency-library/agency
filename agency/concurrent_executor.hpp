...
class concurrent_executor
{
    ...
    std::future<void> async_execute(Function f, size_t n, T shared_arg)
    {
      ...
      if(n > 0)
      {
        result = std::async(std::launch::async, [=]() mutable
        {
          ...
          if(0 < mid)
          {
            left = std::move(this->async_execute(f, 0, mid, shared_arg));
          }

          std::future<void> right = detail::make_ready_future();
          if(mid + 1 < n)
          {
            right = std::move(this->async_execute(f, mid + 1, n, shared_arg));
          }
  ...
  private:
    ...
    template<class Function, class T>
    std::future<void> async_execute(Function f, size_t first, size_t last, T& shared_arg)
    {
      return std::async(std::launch::async, [=,&shared_arg]() mutable
      {
        ...
        if(first < mid)
        {
          left = std::move(this->async_execute(f, first, mid, shared_arg));
        }

        std::future<void> right = detail::make_ready_future();
        if(mid + 1 < last)
        {
          right = std::move(this->async_execute(f, mid + 1, last, shared_arg));
        }
