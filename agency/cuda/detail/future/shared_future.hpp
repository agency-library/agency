...
class shared_future
{
  ...
  public:
    ...
    agency::cuda::future<agency::detail::result_of_t<ResultFactory()>>
      new_bulk_then(Function f, Shape shape, IndexFunction index_function, ResultFactory result_factory, OuterFactory outer_factory, InnerFactory inner_factory, agency::cuda::device_id device)
    {
      return underlying_future_->new_bulk_then_and_leave_valid(f, shape, index_function, result_factory, outer_factory, inner_factory, device);
    }
};
