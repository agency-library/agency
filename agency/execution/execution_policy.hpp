...
/// \brief Encapsulates requirements for creating groups of vector execution agents.
/// \ingroup execution_policies
///
///
/// When used as a control structure parameter, `vector_execution_policy` requires the creation of a group of execution agents which execute without any order.
///
/// The type of execution agent `vector_execution_policy` induces is `vector_agent`, and the type of its associated executor is `vector_executor`.
///
/// \see execution_policies
/// \see basic_execution_policy
/// \see vec
/// \see vector_agent
/// \see vector_executor
/// \see vector_execution_tag
// XXX need to rename this and everything else to use the unsequenced nomenclature
...
/// \brief The global variable `vec` is the default `vector_execution_policy`.
/// \ingroup execution_policies
