...
void test()
{
  ...
  {
    // async_execute with shared inits returning user-specified container
    ...
    std::mutex mut;
    std::vector<int> result = agency::new_executor_traits<executor_type>::template async_execute<std::vector<int>>(exec, [&mut](size_t idx, int& shared_arg)
    {
      mut.lock();
      ++shared_arg;
      mut.unlock();
  ...
  {
    // async_execute with shared inits returning default container
  ...
  {
    // async_execute with shared inits returning void
