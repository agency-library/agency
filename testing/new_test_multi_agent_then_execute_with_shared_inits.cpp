...
void test()
{
...

  {
    ...
    std::mutex mut;
    std::future<std::vector<int>> fut = agency::new_executor_traits<executor_type>::new_then_execute(exec, [&](size_t idx, int& past, int& shared_arg)
    {
      mut.lock();
      ++shared_arg;
      mut.unlock();
...

  {
    ...
    std::mutex mut;
    std::future<std::vector<int>> fut = agency::new_executor_traits<executor_type>::new_then_execute(exec, [&](size_t idx, int& shared_arg)
    {
      mut.lock();
      ++shared_arg;
      mut.unlock();
