...
void test_returning_void(Executor exec)
{
  ...
  std::mutex mut;
  agency::detail::bulk_sync_execute_with_auto_result(exec, [&](size_t, size_t& shared_arg)
  {
    mut.lock();
    increment_me += 1;
    ++shared_arg;
...
void test_returning_void2(Executor exec)
{
  ...
  agency::detail::bulk_sync_execute_with_auto_result(exec, [] __device__ (index_type, int& outer_shared_arg, int& inner_shared_arg)
  {
