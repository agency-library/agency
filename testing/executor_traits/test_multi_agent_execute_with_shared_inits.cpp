...
void test()
{
...

  {
    ...
    std::mutex mut;
    std::vector<int> result = agency::executor_traits<executor_type>::execute(exec, [&mut](size_t idx, int& shared_arg)
    {
      mut.lock();
      ++shared_arg;
      mut.unlock();
...

  {
    ...
    std::mutex mut;
    auto result = agency::executor_traits<executor_type>::execute(exec, [&mut](size_t idx, int& shared_arg)
    {
      mut.lock();
      ++shared_arg;
      mut.unlock();
...

  {
    ...
    int increment_me = 0;
    std::mutex mut;
    agency::executor_traits<executor_type>::execute(exec, [&](size_t idx, int& shared_arg)
    {
      mut.lock();
      ++shared_arg;
      increment_me += 13;
